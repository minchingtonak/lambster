	...
	it('Basic clone test', () => {
		...
		orig_terms.forEach((_, i) => {
			...
			if (orig_terms[i] instanceof Abstraction) {
				...
				expect((orig_terms[i] as Variable).name).to.equal(
					(copy_terms[i] as Variable).name
				);
