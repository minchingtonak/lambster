...
import { Application, Abstraction, Term } from '../src/ast';
...
describe('Parser tests', () => {
	...
	it('Associativity test 1', () => {
		expect(
			stringify(
				new Parser(
					new Lexer('Lx.x x x x', logger).lexTokens(),
					logger
				).parseTerm() as Term
			)
		).to.equal('(λx. (((x x) x) x))');
	...
	it('Error test 2', () => {
		expect(
			new Parser(
				new Lexer('Lx.x x L x x', logger).lexTokens(),
				logger
			).parseTerm()
		).to.be.a('null');
	});
	...
	it('Error test 3', () => {
		expect(
			new Parser(
				new Lexer('Lx.. z x x', logger).lexTokens(),
				logger
			).parseTerm()
		).to.be.a('null');
	});
	...
	it('Error test 7', () => {
		expect(
			new Parser(
				new Lexer('Lx world. z x ) x', logger).lexTokens(),
				logger
			).parseTerm()
		).to.be.a('null');
	});
