...
export class Reducer implements TermVisitor<Term> {
	private redex: Term = new Variable('_default', -1);
	private depth = 0;

	private static MAX_RECURSION_DEPTH = 1000;
	...
	private reduce(term: Term): Term {
		if (this.depth++ > Reducer.MAX_RECURSION_DEPTH) {
	...
	visitApplication(application: Application): Term {
		const f_normal: Term = (application.func = this.reduce(application.func)),
			x_normal: Term = (application.argument = this.reduce(
		...
		const conflicting_abs = new Set<Abstraction | null>(
			x_normal
				.getAllBoundVars()
		...
		conflicting_abs.forEach(abs => {
			...
			this.logger.vvlog(
				`Alpha reducing '${stringify(abs)}' with name '${new_name}'`
			);
			abs.alphaReduce(new_name);
		});

		if (conflicting_abs.size !== 0) {
			this.logger.vlog(`α > ${stringify(this.redex)}`);
		}
	...
	private current_name_prefix = 0;
	private genNewName(): string {
		return `X${this.current_name_prefix++}`;
	}

	private current_free_name_prefix = 0;
