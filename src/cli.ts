...
function parseArgs() {
	const options: InterpreterOptions & { help?: boolean; filename?: string } = {
	...
	process.argv.slice(2).forEach(arg => {
		if (arg.startsWith('-')) {
			switch (arg.substring(1)) {
				case 'v':
					if (options.verbosity ?? Verbosity.NONE < Verbosity.LOW) {
				...
				default:
					throw new Error(
						`Failed to parse arguments: unexpected option '${arg}'`
					);
			}
		} else {
