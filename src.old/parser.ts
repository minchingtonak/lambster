...
export class Parser {
	...
	private stmt(): Stmt | null {
		try {
			...
			} else if (this.match(TokenType.ENV)) {
				return this.cmdStmt(TokenType.ENV, 'env');
			} else if (this.match(TokenType.HELP)) {
				return this.cmdStmt(TokenType.HELP, 'help');
			} else if (this.match(TokenType.UNBIND)) {
				return this.unbindStmt();
			}
	...
	private bindingStmt(): BindingStmt {
		...
		this.consume(
			TokenType.NEWLINE,
			'Expected newline after binding statement.'
		);
		return new BindingStmt(ident, term);
	}

	private static TOKEN_TO_CMD: CommandType[] = [
		CommandType.HELP,
		CommandType.HELP,
		CommandType.HELP,
		CommandType.ENV,
	...
	private cmdStmt(type: TokenType, str: string): CommandStmt {
		...
		return new CommandStmt(Parser.TOKEN_TO_CMD[type]);
	}
	...
	private term(): Term {
		return this.match(TokenType.LAMBDA)
			? this.abstraction()
			: this.application();
	}
	...
	private abstraction(): Term {
		...
		while (this.peek().type === TokenType.IDENTIFIER) {
			idents.push(this.advance().lexeme);
		}
		...
		let abs: Abstraction = new Abstraction(
			innermost,
			this.idList.get(innermost),
			this.term()
		);
	...
	private atom(): Term {
		...
		if (this.match(TokenType.IDENTIFIER)) {
			...
			return new Variable(
				name,
				this.idList.has(name) ? this.idList.get(name) : 0
			);
		}
