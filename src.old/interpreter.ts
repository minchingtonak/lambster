...
import { join, stringify } from './utils';
...
export class Interpreter implements StmtVisitor<void> {
	...
	private addHash(term: Term, name: string) {
		...
		if (!(s_hash in this.structure_hashes)) {
			this.structure_hashes[s_hash] = new Set<string>();
		}
	...
	private start_index = -1; // initialized by the IIFE below
	private bindings: { [key: string]: Term } =
		...
		(dict => {
			...
			Object.keys(dict).forEach(key => {
				parser.setTokens(new Lexer(dict[key as keyof typeof dict]).lexTokens());
	...
	private rename_free_vars = false;
	...
	setOptions(options?: InterpreterOptions) {
		...
		if (!this.logger) {
			this.logger = new Logger({
				...
				output_stream:
					(options ? options.output_stream : undefined) || process.stdout,
			});
	...
	evaluate(source: string) {
		...
		const stmts = new Parser(
			new Lexer(source, this.logger).lexTokens(),
			this.logger,
			this.start_index
		).parse();

		if (this.logger.hasError || !stmts) {
			return;
		}
	...
	visitCommandStmt(command: CommandStmt): void {
		switch (command.type) {
			...
			case CommandType.UNBIND:
				this.deleteBinding(command.argument as string);
				break;
	...
	hadError(): boolean {
		return this.logger?.hasError ?? true;
	}

	private eval(term: Term, binding_name?: string): Term | null {
		if (!this.logger || !this.resolver) {
			return null;
		}
		...
		try {
			const reduct: Term = new Reducer(
				this.rename_free_vars,
				this.logger
			).reduceTerm(this.resolver.resolveTerm(term));
			this.logger.vvlog();
			if (binding_name) {
				this.logger.log(`>>> ${binding_name} = ${stringify(reduct)}`);
			} else {
				this.logger.log(`>>> ${stringify(reduct)}`);
			}
			const s_hash = structureHash(reduct);
			if (s_hash in this.structure_hashes) {
				this.logger.log(
					`    ↳ equivalent to: ${join(this.structure_hashes[s_hash], ', ')}`
				);
			}
			this.logger.vlog();
			return reduct;
